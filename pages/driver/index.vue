...
<v-toolbar-title dark>Drivers</v-toolbar-title>
<v-divider
class="mx-4"
inset
vertical
...
<v-btn color="primary" to="driver/create" class="accent--text mb-2">New Driver</v-btn>
</template>
<v-card>
<!-- <v-card-title>
...
this.$axios.delete('driver/'+item.id)
.then((res) => {
...
this.$axios.put('driver/' + this.editedItem.id,payload)
.then(res => {
if(res.data.response_status){
const index = this.drivers.findIndex(item => item.id == this.editedItem.id)
Object.assign(this.drivers[index],res.data.updated_record);
this.snackbar = res.data.response_status;
this.response.msg = res.data.message;
this.close()
}
