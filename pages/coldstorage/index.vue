...
<v-toolbar-title dark>Cold Storage</v-toolbar-title>
<v-divider
class="mx-4"
inset
vertical
...
<v-btn color="primary" to="coldstorage/create" class="accent--text mb-2">New Item</v-btn>
</template> -->
<v-card>
<!-- <v-card-title>
...
<v-row>
<v-col>
<v-text-field type="number" :rules="Rules" v-model="editedItem.phone_number" label="Phone Number"></v-text-field>
</v-col>
<v-col>
<v-text-field type="number" :rules="Rules" v-model="editedItem.mobile_number" label="Mobile Number"></v-text-field>
</v-col>
</v-row>
<v-row>
<v-col>
...
// await store.dispatch('getcoldstorage')       
// },
data: () => ({
...
//   },
// coldstorage(){
//     return this.$store.state.coldstorage
// }
...
const coldstorage = await this.$axios.get('coldstorage');
this.coldstorage = coldstorage.data;
...
editItem (item) {
this.editedIndex = this.coldstorage.indexOf(item)
...
this.$axios.delete('coldstorage/'+item.id)
.then((res) => {
if(res.data.response_status){
const index = this.coldstorage.indexOf(item)
this.coldstorage.splice(index, 1)
...
this.$axios.put('coldstorage/' + this.editedItem.id,payload)
.then(res => {
if(res.data.response_status){
const index = this.coldstorage.findIndex(item => item.id == this.editedItem.id)
this.coldstorage.splice(index, 1, res.data.updated_record);
this.snackbar = res.data.response_status;
this.response.msg = res.data.message;
this.close()
}
